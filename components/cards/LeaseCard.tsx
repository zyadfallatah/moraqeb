import { License, Subscription } from "@/database/schema";
import React from "react";
import Image from "next/image";
import { getLicenseNotices } from "@/lib/actions/noticeActions";
import { cn } from "@/lib/utils";
import Link from "next/link";
import OnlySubscribed from "../shared/OnlySubscribed";
...
const LeaseCard = async ({ lease, subscription }: Props) => {
  const notice = await getLicenseNotices(lease.licenseNumber);
  const { color, text } = mapState[notice?.[0]?.type ?? "info"];

  return (
    <OnlySubscribed
      byLicnese={lease.licenseNumber}
      isActiveSubscription={subscription !== null}
    >
